...
public class WallVisibilityControllerWithHurtedPlayerMap : MonoBehaviour
{
    ...
    public GameObject Voda; // Ссылка на объект Voda
    public GameObject Eat; // Ссылка на объект Eat
    public GameObject Battery; // Ссылка на объект Battery
    ...
    void Update()
    {
        ...
        // Проверяем состояние объектов Voda, Eat и Battery
        if (Voda != null && Eat != null && Battery != null && WallBlock2 != null)
        {
            if (!Voda.activeSelf && !Eat.activeSelf && !Battery.activeSelf)
            {
